...
\textbf{Note}: The prompts shown below are \textit{simplified summaries}. The actual prompts used in experiments were substantially larger (30--80 lines) and included pre-execution shell commands for context gathering. Full prompt text is available at:

\begin{center}
\url{https://github.com/rhowardstone/aTSR/tree/main/src/prompts}
\end{center}
...
\midrule
V1: Baseline & 30 & Simple directive to improve coverage. No pre-execution context. \\
V2: +Context & 40 & Adds pre-execution commands: \texttt{find}, \texttt{wc}, \texttt{ls} to detect language, structure, test locations, code/test ratio. \\
V3: +Coverage & 60 & V2 + explicit \texttt{coverage.py} workflow with iterative gap-filling loop. \\
V4: +Mutations & 60 & V2 + explicit \texttt{mutmut} workflow for mutation-guided testing. \\
V5: +Both & 80 & V2 + three-phase workflow: coverage first, then mutations, then verification. \\
V6: Minimal & 15 & Pre-execution context only. Directive reduced to: ``Improve coverage to 80\%+.'' \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Phase 1 (Base vs Refine)}: ``Base'' used a simple coverage improvement prompt; ``Refine'' used the full \texttt{/refine-tests} slash command implementing a 6-phase workflow (environment detection, coverage analysis, mutation testing, recommendations, property testing, verification). Full slash command source: \url{https://github.com/rhowardstone/aTSR/tree/main/.claude/commands}
...
\begin{itemize}
\item Claude Sonnet 4.5 (claude-sonnet-4-5-20250901) -- aTSR experiments
\item Claude Opus 4.1 (claude-opus-4-1-20250901) -- aTSR experiments
