...
% Rename tables and figures to S1, S2, etc. to avoid overlap with main paper
...
\url{https://github.com/rhowardstone/aTSR/blob/main/src/stat_tests.py}

\textbf{Test 1: Sign test for V4 underperforming V1.} For each of 9 repositories, we compared V4 coverage to V1 coverage. V4 was worse in all 9 cases. Under the null hypothesis (no difference), each repo has 50\% chance of V4 being worse. Using \texttt{scipy.stats.binomtest(9, 9, p=0.5)}, we obtain p=0.002.

\textbf{Test 2: Variance decomposition.} We computed between-group variance for repositories (132.0) and for prompt variants (5.2), yielding a ratio of 25.5$\times$. Repository characteristics explain 87\% of total variance; prompt strategy explains 3\%.

\textbf{Test 3: Token efficiency (V5 vs V6).} We hypothesized V6 (Minimal) would use fewer tokens than V5 (+Both). Testing on 6 general-purpose repos, V6 used fewer tokens in only 2/6 cases. Sign test yields p=0.89---not significant.

\textbf{Test 4: Domain effects.} Comparing V1 coverage between bioinformatics repos (mean 60\%) and general repos (mean 83\%) using Mann-Whitney U test yields p=0.19---not significant, likely due to small sample size (n=3 bio repos).
...
In 5 of 9 repositories, V4 (+Mutations) achieved $\leq$10\% final coverage. This is not occasional underperformance---it is systematic failure. On click, V4 achieved 0\% coverage because mutation testing never completed and the agent became trapped in setup. On python-box, V4 achieved only 1\% coverage when mutmut encountered errors on python-box's complex nested dictionary types. The bioinformatics repositories fared even worse: dnaapler achieved 0\% coverage because bioinformatics domain concepts confused the mutation analysis, fastqe achieved 0\% coverage because emoji handling in test assertions created mutation chaos, and pyfaidx achieved 0\% coverage because FASTA file format parsing generated hundreds of trivial mutants.
