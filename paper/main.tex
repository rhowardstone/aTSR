...
Software engineers have developed sophisticated tools for analyzing and improving code quality. Code coverage tools such as coverage.py and gcov identify which lines are executed by tests. Mutation testers like mutmut and Stryker verify whether tests actually detect code changes. Static analyzers including pyright and Infer find potential bugs without execution, while AST parsers such as Tree-sitter enable structural code analysis.
...
\begin{enumerate}
    \item \textbf{aTSR}: An open-source Claude Code slash command implementing multi-phase test refinement with coverage analysis and mutation testing
...
Our exploratory study (n=1 per configuration) yielded statistically-supported findings. Most critically, mutation testing can catastrophically fail: V4 (+Mutations) achieved $\leq$10\% coverage in 5/9 repos (p=0.002). Variance decomposition revealed that repository characteristics explained 25.5$\times$ more variance than prompt strategy choice. Despite this, simple baselines remained competitive---a minimal prompt matched elaborate workflows in most configurations. Finally, the JobsCoach case study demonstrated that coverage does not equal correctness: 6 bugs escaped high-coverage backend tests.

\textit{While based on single runs per configuration, these patterns---particularly the mutation testing failure and variance decomposition---are statistically robust and merit attention.}
...
\toprule
\textbf{Finding} & \textbf{Stat} & \textbf{p} & \textbf{Interpretation} \\
\midrule
V4 $<$ V1 cov. & 9/9 & 0.002 & Mutations hurt$^*$ \\
Repo variance & 25.5$\times$ & --- & Repo $>>$ prompt \\
V6 tokens & 2/6 & 0.89 & Not significant \\
Bio vs general & 60/83\% & 0.19 & Not significant \\
\bottomrule
\end{tabular}

\vspace{1mm}
\noindent{\footnotesize $^*$Requires replication; pattern robust but magnitude may vary.}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/testscout-som.png}
...
\vspace{1mm}
\noindent{\footnotesize $^*$Anomaly: click repository had 190 pre-existing tests; Opus/base spent extra tokens analyzing these before writing new tests (Opus/refine handled this efficiently).}
\end{table}

Several patterns emerged from Phase 1 trials. Base matched or exceeded refine coverage in 10 of 12 configurations and showed higher pass rates, often reaching 100\%. Sonnet performed comparably to Opus at lower cost. Meanwhile, the refine strategy used 40--70\% more tokens without apparent coverage benefit.

\textit{Note: With n=1, these patterns may reflect run-to-run variance rather than true strategy differences.}
...
Analysis of tool calls revealed interesting patterns. V4 and V5 used more Bash commands (70--80 per run) than V6 (40--50), primarily for running mutmut. V6 used fewer Read calls, suggesting it wrote tests more directly rather than extensively analyzing the codebase. Interestingly, all variants used TodoWrite similarly, indicating task management was prompt-agnostic.
